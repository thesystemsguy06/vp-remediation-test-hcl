# matrix/sb2_glue — Glue ETL job with NO security_configuration attached, so job
# bookmarks / CloudWatch logs / S3 shuffle data are unencrypted. Targets:
...
resource "aws_glue_job" "vp" {
  name     = "vp-sb2-glue-${random_id.s.hex}"
  ...
  command {
    name            = "glueetl"
    script_location = "s3://vp-test-data-746210888062/glue/vp-sb2-${random_id.s.hex}.py"
    python_version  = "3"
  }
